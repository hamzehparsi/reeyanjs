...
// Fetch data
const { data: contentType, pending, error } = await useFetch(
...
const columns = [
  {
    accessorKey: 'title',
    header: 'عنوان',
    cell: ({ row }) => h('div', { class: 'text-wrap min-w-64' }, row.original.title)
  },
  {
    ...
    cell: ({ row }) => new Date(row.original.date).toLocaleDateString("fa-IR") // تغییر از createdAt به date
  },
  {
    id: 'action',
    header: 'عملیات',
    cell: ({ row }) => h('div', { class: 'flex gap-4' }, [
      h(resolveComponent('UButton'), {
        class: 'bg-transparent border border-slate-200 px-6 py-2 rounded-full text-slate-400 hover:bg-sabz-light hover:text-white',
        onClick: () => handleEdit(row.original._id),
        label: 'ویرایش'
      }),
      h(resolveComponent('UButton'), {
        class: 'bg-transparent border border-red-200 px-6 py-2 rounded-full text-red-600 hover:bg-red-600 hover:text-white',
...
async function deleteContent() {
  try {
    ...
    await $fetch(`/api/entries/${collectionName}/${selectedItemToDelete.value}`, {
      method: "DELETE",
    });
    ...
    toast.add({ color: "error", title: "خطا", description: "خطا در حذف محتوا" });
...
<template>
  <div class="flex items-center justify-between pr-2">
    <div class="flex items-center gap-1 divide-x divide-gray-400">
      <div v-if="contentType" class="flex items-center gap-1 pl-4 font-black text-lg">
        <span>لیست</span>
    ...
    <NuxtLink v-if="contentType?.fields.length" :to="`/admin/collection/${collectionName}/create`"
      class="flex items-center gap-1 bg-blue px-4 py-2 rounded-lg text-white">
      <span>+</span>
      <span>درج <span>{{ contentType.displayName }}</span></span>
    </NuxtLink>
  </div>
  <div class="flex items-center gap-4 mt-4">
    <UInput v-model="globalFilter" class="max-w-sm" size="xl"
      variant="outline" placeholder="جستجو ..." />
  </div>
  <div v-if="paginatedItems.length > 0" class="mt-4 bg-white rounded-xl px-7 py-4">
    <div class="w-full flex flex-col">
      <ClientOnly>
        <UTable v-model:sorting="sorting" :data="paginatedItems" :columns="columns" class="w-full" />
      </ClientOnly>
      <div class="flex justify-center pt-4 mt-4">
        <UPagination v-model="pagination.pageIndex" :page-count="Math.ceil(filteredItems.length / pagination.pageSize)"
          :total="filteredItems.length" :items-per-page="pagination.pageSize"
          @update:page="(page) => (pagination.pageIndex = page - 1)" firstIcon="i-lucide-chevrons-right"
          prevIcon="i-lucide-arrow-right" nextIcon="i-lucide-arrow-left" lastIcon="i-lucide-chevrons-left" />
      </div>
    </div>
  </div>
  <div v-else class="bg-white rounded-xl flex h-96 w-full justify-center flex-col">
    <IconsNoField class="size-42 mx-auto" />
    <div v-if="contentType" class="text-sm text-blue-dark -mt-8 text-center flex flex-col items-center gap-3">
      <span class="text-xs">هنوز هیچ {{ contentType.displayName }} ثبت نشده است
      </span>
      <NuxtLink :to="`/admin/collection/${collectionName}/create`"
        ...
        <span class="pt-1 text-xs"> اضافه کردن {{ contentType.displayName }} </span>
      </NuxtLink>
    </div>
  </div>
  <UModal :ui="{
